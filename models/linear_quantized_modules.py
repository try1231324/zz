...
def symmetric_linear_quantization_scale_factor(num_bits, saturation_val):
    # Leave one bit for sign scale计算，saturation_val是数据的最大值
    n = 2 ** (num_bits - 1) - 1
...
def asymmetric_linear_quantization_scale_factor(num_bits, saturation_min, saturation_max):
    n = 2 ** num_bits - 1
    return n / (saturation_max - saturation_min) #非对称量化的scale
...
def clamp(input, min, max, inplace=False):
    if inplace:
        ...
        return input
    return torch.clamp(input, min, max) #截断函数
...
def linear_quantize(input, scale_factor, inplace=False):
    if inplace:
        ...
        return input
    return torch.floor(scale_factor * input) #量化
...
def linear_quantize_clamp(input, scale_factor, clamp_min, clamp_max, inplace=False):
    ...
    return clamp(output, clamp_min, clamp_max, inplace) #量化截断
...
def linear_dequantize(input, scale_factor, inplace=False):
    if inplace:
        ...
        return input
    return input / scale_factor #反量化


def get_tensor_max_abs(tensor):
    return max(abs(tensor.max().item()), abs(tensor.min().item())) #计算张量最大绝对值


def get_tensor_avg_max_abs_across_batch(tensor): #计算批量中每个样本的最大值的平均值。
...
def get_quantized_range(num_bits, signed=True):
    if signed:
        n = 2 ** (num_bits - 1)
        return -n, n - 1
    return 0, 2 ** num_bits - 1  #给定比特宽度计算量化范围
...
class LinearQuantizeSTE(torch.autograd.Function): #STE
    @staticmethod
